...
<!-- UsageSnippet language="typescript" operationID="checkdatasourceauth" method="post" path="/rest/api/v1/checkdatasourceauth" -->
```typescript
...
import { clientAuthenticationCheckDatasourceAuth } from "@gleanwork/api-client/funcs/clientAuthenticationCheckDatasourceAuth.js";
...
async function run() {
  const res = await clientAuthenticationCheckDatasourceAuth(glean);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("clientAuthenticationCheckDatasourceAuth failed:", res.error);
  }
}
...
import {
  ...
  useClientAuthenticationCheckDatasourceAuthMutation
} from "@gleanwork/api-client/react-query/clientAuthenticationCheckDatasourceAuth.js";
```
