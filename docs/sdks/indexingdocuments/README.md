...
```typescript
import { GleanCore } from "@gleanwork/api-client/core.js";
import { indexingDocumentsDebugEvents } from "@gleanwork/api-client/funcs/indexingDocumentsDebugEvents.js";
...
async function run() {
  const res = await indexingDocumentsDebugEvents(glean, {
    objectType: "Article",
    docId: "art123",
    startDate: "2025-05-01",
    maxEvents: 50,
  ...
  if (res.ok) {
    ...
    console.log("indexingDocumentsDebugEvents failed:", res.error);
  }
}
...
import {
  // Mutation hook for triggering the API call.
  useIndexingDocumentsDebugEventsMutation
} from "@gleanwork/api-client/react-query/indexingDocumentsDebugEvents.js";
```
