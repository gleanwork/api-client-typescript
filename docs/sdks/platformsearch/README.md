...
import { platformSearchQuery } from "@gleanwork/api-client/funcs/platformSearchQuery.js";
...
async function run() {
  const res = await platformSearchQuery(glean, {
    query: "quarterly planning 2026",
    datasources: [
      "confluence",
      "google_drive",
  ...
  if (res.ok) {
    ...
    console.log("platformSearchQuery failed:", res.error);
  }
}
...
import {
  ...
  usePlatformSearchQueryMutation
} from "@gleanwork/api-client/react-query/platformSearchQuery.js";
```
