 ...
 * exactly.
 */
export function clientToolsGetToolServerTools(
  client: GleanCore,
  serverId: string,
...
> {
  const input: operations.GetToolServerToolsRequest = {
    serverId: serverId,
    toolNames: toolNames,
  };
  ...
  const parsed = safeParse(
    input,
    (value) => operations.GetToolServerToolsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rest/api/v1/tool-servers/{serverId}/tools")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ToolDefinitionsResponse$inboundSchema),
