 ...
 */
export function platformAgentsGet(
  client: GleanCore,
  agentId: string,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PlatformAgentGetResponse,
...
> {
  const input: operations.PlatformAgentsGetRequest = {
    agentId: agentId,
  };

  const parsed = safeParse(
    input,
    (value) => operations.PlatformAgentsGetRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/agents/{agent_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformAgentGetResponse$inboundSchema),
    M.jsonErr(
