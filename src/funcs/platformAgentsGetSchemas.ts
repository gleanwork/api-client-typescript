 ...
 */
export function platformAgentsGetSchemas(
  client: GleanCore,
  agentId: string,
...
> {
  const input: operations.PlatformAgentsGetSchemasRequest = {
    agentId: agentId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformAgentsGetSchemasRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/agents/{agent_id}/schemas")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformAgentSchemasResponse$inboundSchema),
    M.jsonErr(
