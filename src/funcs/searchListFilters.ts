...
> {
  const input: operations.PlatformSearchFiltersRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformSearchFiltersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/search/filters")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformSearchFiltersResponse$inboundSchema),
    M.jsonErr(
