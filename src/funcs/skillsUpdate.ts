...
export function skillsUpdate(
  ...
  Result<
    components.PlatformSkillUpdateResponse,
...
> {
  const input: operations.PlatformSkillsUpdateRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformSkillsUpdateRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PlatformSkillUpdateRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/skills/{skill_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformSkillUpdateResponse$inboundSchema),
    M.jsonErr(
