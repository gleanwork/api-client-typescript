 ...
 */
export function triggersListPresets(
  client: GleanCore,
...
> {
  const input: operations.PlatformTriggerPresetsListRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformTriggerPresetsListRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/trigger-presets")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformTriggerPresetListResponse$inboundSchema),
    M.jsonErr(
      [400, 401, 403, 408, 429],
