 ...
 */
export function triggersSearchEvents(
  client: GleanCore,
  triggerId: string,
...
> {
  const input: operations.PlatformTriggersEventsSearchRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformTriggersEventsSearchRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.PlatformTriggerEventSearchRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/triggers/{trigger_id}/events/search")(
    pathParams,
  );
