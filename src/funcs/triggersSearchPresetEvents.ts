 ...
 */
export function triggersSearchPresetEvents(
  client: GleanCore,
  presetId: string,
...
> {
  const input: operations.PlatformTriggerPresetsEventsSearchRequest = {
    preset_id: presetId,
    platformTriggerPresetEventSearchRequest:
      platformTriggerPresetEventSearchRequest,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PlatformTriggerPresetsEventsSearchRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload.PlatformTriggerPresetEventSearchRequest,
    { explode: true },
  );
  ...
  const path = pathToFunc("/api/trigger-presets/{preset_id}/events/search")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PlatformTriggerEventSearchResponse$inboundSchema),
    M.jsonErr(
